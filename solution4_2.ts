...
const main = () => {
  ...
  let winningBoards = 0;
  let remBoards = boards;

  for (const drawn of drawnNumbers) {
    remBoards.forEach(board => markBoard(board, drawn));

    const winners = remBoards.filter(board => checkWin(board));
    for (const winner of winners) {
      winningBoards++;
      if (winningBoards === numBoards) {
    ...
    const losers = remBoards.filter(board => !checkWin(board));
    remBoards = losers;
  }
};
