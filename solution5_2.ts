...
const main = () => {
  ...
  for (const line of lines) {
    ...
    if (x1 === x2 && y1 === y2) {
      ...
      let lowerX = x1 < x2 ? [x1, y1] : [x2, y2];
      const higherX = x1 > x2 ? [x1, y1] : [x2, y2];

      if (lowerX[1] < higherX[1]) {
        while (lowerX[0] <= higherX[0] || lowerX[1] <= higherX[1]) {
          grid[`${lowerX[0]},${lowerX[1]}`] = (grid[`${lowerX[0]},${lowerX[1]}`] ?? 0) + 1;
          lowerX[0]++;
          lowerX[1]++;
        }
      } else {
